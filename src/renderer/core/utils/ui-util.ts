export class UiUtil {
  public static navigateGrid(
    ...
    const active = (
      direction == "up" || direction == "left" ? selected[0] : selected[selected.length - 1]
    ) as HTMLElement;
    ...
    const isRightColumn =
      activeIndex % numPerRow === numPerRow - 1 || activeIndex === gridChildrenCount - 1;

    // Select the next item
    switch (direction) {
      case "up":
        if (!isTopRow) select(activeIndex - numPerRow);
        break;
      case "down":
        if (!isBottomRow) select(activeIndex + numPerRow);
        break;
      case "left":
        if (!isLeftColumn) select(activeIndex - 1);
        break;
      case "right":
        if (!isRightColumn) select(activeIndex + 1);
        break;
    }
  }
